...
public class DiffUtil {
    private static final Comparator<Snake> SNAKE_COMPARATOR = new Comparator<Snake>() { // from class: androidx.recyclerview.widget.DiffUtil.1
        ...
        public int compare(Snake snake, Snake snake2) {
            int i = snake.f23x - snake2.f23x;
            return i == 0 ? snake.f24y - snake2.f24y : i;
        }
    };
    ...
    public static DiffResult calculateDiff(Callback callback, boolean z) {
        ...
        while (!arrayList2.isEmpty()) {
            ...
            if (diffPartial != null) {
                ...
                diffPartial.f23x += range.oldListStart;
                diffPartial.f24y += range.newListStart;
                Range range2 = arrayList3.isEmpty() ? new Range() : (Range) arrayList3.remove(arrayList3.size() - 1);
                range2.oldListStart = range.oldListStart;
                range2.newListStart = range.newListStart;
                if (diffPartial.reverse) {
                    range2.oldListEnd = diffPartial.f23x;
                    range2.newListEnd = diffPartial.f24y;
                } else if (diffPartial.removal) {
                    range2.oldListEnd = diffPartial.f23x - 1;
                    range2.newListEnd = diffPartial.f24y;
                } else {
                    range2.oldListEnd = diffPartial.f23x;
                    range2.newListEnd = diffPartial.f24y - 1;
                }
                arrayList2.add(range2);
                if (diffPartial.reverse) {
                    if (diffPartial.removal) {
                        range.oldListStart = diffPartial.f23x + diffPartial.size + 1;
                        range.newListStart = diffPartial.f24y + diffPartial.size;
                    } else {
                        range.oldListStart = diffPartial.f23x + diffPartial.size;
                        range.newListStart = diffPartial.f24y + diffPartial.size + 1;
                    }
                } else {
                    range.oldListStart = diffPartial.f23x + diffPartial.size;
                    range.newListStart = diffPartial.f24y + diffPartial.size;
    ...
    */
    ...
    public static class DiffResult {
        ...
        private void addRootSnake() {
            ...
            if (snake != null && snake.f23x == 0 && snake.f24y == 0) {
                return;
            }
        ...
        private void findMatchingItems() {
            ...
            for (int size = this.mSnakes.size() - 1; size >= 0; size--) {
                ...
                int i3 = snake.f23x + snake.size;
                int i4 = snake.f24y + snake.size;
                ...
                for (int i5 = 0; i5 < snake.size; i5++) {
                    int i6 = snake.f23x + i5;
                    int i7 = snake.f24y + i5;
        ...
        private boolean findMatchingItem(int i, int i2, int i3, boolean z) {
            ...
            while (i3 >= 0) {
                ...
                int i7 = snake.f23x + snake.size;
                int i8 = snake.f24y + snake.size;
                if (z) {
        ...
        public void dispatchUpdatesTo(ListUpdateCallback listUpdateCallback) {
            ...
            for (int size = this.mSnakes.size() - 1; size >= 0; size--) {
                Snake snake = this.mSnakes.get(size);
                int i3 = snake.size;
                int i4 = snake.f23x + i3;
                int i5 = snake.f24y + i3;
                if (i4 < i) {
                ...
                for (int i6 = i3 - 1; i6 >= 0; i6--) {
                    if ((this.mOldItemStatuses[snake.f23x + i6] & 31) == 2) {
                        batchingListUpdateCallback.onChanged(snake.f23x + i6, 1, this.mCallback.getChangePayload(snake.f23x + i6, snake.f24y + i6));
                    }
                }
                i = snake.f23x;
                i2 = snake.f24y;
