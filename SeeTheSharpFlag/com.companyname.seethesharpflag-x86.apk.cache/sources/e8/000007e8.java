...
public class PathInterpolatorCompat implements Interpolator {
    ...
    private void initPath(Path path) {
        ...
        this.f34mX = new float[min];
        this.f35mY = new float[min];
        float[] fArr = new float[2];
        for (int i2 = 0; i2 < min; i2++) {
            pathMeasure.getPosTan((i2 * length) / (min - 1), fArr, null);
            this.f34mX[i2] = fArr[0];
            this.f35mY[i2] = fArr[1];
        }
        if (Math.abs(this.f34mX[0]) <= 1.0E-5d && Math.abs(this.f35mY[0]) <= 1.0E-5d) {
            int i3 = min - 1;
            if (Math.abs(this.f34mX[i3] - 1.0f) <= 1.0E-5d && Math.abs(this.f35mY[i3] - 1.0f) <= 1.0E-5d) {
                float f = 0.0f;
                int i4 = 0;
                while (i < min) {
                    float[] fArr2 = this.f34mX;
                    int i5 = i4 + 1;
                    float f2 = fArr2[i4];
                    if (f2 < f) {
        ...
        sb.append(this.f34mX[0]);
        sb.append(",");
        sb.append(this.f35mY[0]);
        sb.append(" end:");
        int i6 = min - 1;
        sb.append(this.f34mX[i6]);
        sb.append(",");
        sb.append(this.f35mY[i6]);
    ...
    public float getInterpolation(float f) {
        ...
        int i = 0;
        int length = this.f34mX.length - 1;
        while (length - i > 1) {
            int i2 = (i + length) / 2;
            if (f < this.f34mX[i2]) {
                length = i2;
        ...
        float[] fArr = this.f34mX;
        float f2 = fArr[length] - fArr[i];
        if (f2 == 0.0f) {
            return this.f35mY[i];
        }
        float[] fArr2 = this.f35mY;
        float f3 = fArr2[i];
